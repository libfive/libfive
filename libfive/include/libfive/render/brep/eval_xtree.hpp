...
{
public:
    XTreeEvaluator(const Tree t, int threadNo)
        : XTreeEvaluator(t, std::map<Tree::Id, float>(), threadNo)
    { /* Nothing to do here */ }

    XTreeEvaluator(const Tree t, const std::map<Tree::Id, float>& vars,
        int threadNo)
        : tape(new Tape(t)), array(tape, vars, threadNo),
          interval(tape, vars, threadNo), feature(tape, vars, threadNo)
    { /* Nothing to do here */ }
