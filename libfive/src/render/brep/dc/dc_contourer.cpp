...
template <Axis::Axis A>
void PseudoDCContourer::load(const std::array<const PseudoDCTree2*, 2>& ts)
{
    // Exit immediately if we can prove that there will be no
    // face produced by this edge.
    if (std::any_of(ts.begin(), ts.end(),
        [](const PseudoDCTree2* t){ return t->type != Interval::AMBIGUOUS; }))
    {
        return;
    }
    ...
    /*  See comment in mesh.cpp about selecting a minimum edge */
    const auto index = std::min_element(ts.begin(), ts.end(),
            [](const PseudoDCTree2* a, const PseudoDCTree2* b)
            { return a->leaf->level < b->leaf->level; }) - ts.begin();
...
template void PseudoDCContourer::load<Axis::X>(const std::array<const PseudoDCTree2*, 2>&);
template void PseudoDCContourer::load<Axis::Y>(const std::array<const PseudoDCTree2*, 2>&);
template void PseudoDCContourer::load<Axis::Z>(const std::array<const PseudoDCTree2*, 2>&);
