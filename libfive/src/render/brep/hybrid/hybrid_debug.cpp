...
HybridDebugMesher::~HybridDebugMesher() {
    if (owned) {
        delete eval;
    }
}

// Code copied from hybrid_mesher.cpp, including comments
template <Axis::Axis A>
void HybridDebugMesher::load(const std::array<const HybridTree<3>*, 4>& ts)
{
...
template void HybridDebugMesher::load<Axis::X>(
        const std::array<const HybridTree<3>*, 4>&);
template void HybridDebugMesher::load<Axis::Y>(
        const std::array<const HybridTree<3>*, 4>&);
template void HybridDebugMesher::load<Axis::Z>(
        const std::array<const HybridTree<3>*, 4>&);
