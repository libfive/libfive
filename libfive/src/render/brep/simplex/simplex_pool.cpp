...
static void run(
        ...
        Root<SimplexTree<N>, typename SimplexTree<N>::Leaf>& root,
        std::mutex& root_lock, ProgressWatcher* progress)
{
    ...
    while (!done.load() && !cancel.load())
    {
        ...
        // TODO: Find our local neighbors.  We do this at the last minute to
        ...
        {
            t->evalLeaf(eval, region, tape, spare_leafs);
        }
        ...
        if (progress)
        {
            if (can_subdivide)
            {
                // Accumulate all of the child SimplexTree cells that would have been
...
Root<SimplexTree<N>, typename SimplexTree<N>::Leaf> SimplexTreePool<N>::build(
            ...
            ProgressCallback progress_callback)
{
    auto root(new SimplexTree<N>(nullptr, 0));
    ...
    Root<SimplexTree<N>, typename SimplexTree<N>::Leaf> out(root);
    std::mutex root_lock;
    ...
    if (cancel.load())
    {
        return Root<SimplexTree<N>, typename SimplexTree<N>::Leaf>();
    }
    else
    {
        return out;
