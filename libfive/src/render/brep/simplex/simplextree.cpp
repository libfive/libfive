...
template <unsigned N>
SimplexTree<N>::XTree(XTree<N>* parent, unsigned index)
    : SimplexTree()
{
    reset(parent, index);
}
...
template <unsigned N>
void SimplexTree<N>::reset(XTree<N>* p, unsigned i)
{
    parent = p;
    parent_index = i;
...
template <unsigned N>
void SimplexTree<N>::evalLeaf(XTreeEvaluator* eval, const Neighbors<N>& neighbors,
...
template <unsigned N>
void SimplexTree<N>::releaseChildren(ObjectPool<XTree>& spare_trees,
                               ObjectPool<Leaf>& spare_leafs)
{
    for (auto& c : children)
    {
...
template <unsigned N>
void SimplexTree<N>::saveIntersection(const Vec& pos, const Vec& derivs,
...
template <unsigned N>
uint8_t SimplexTree<N>::buildCornerMask(
        ...
        ObjectPool<SimplexTree<N>>& spare_trees, ObjectPool<Leaf>& spare_leafs)
{
    ...
    bool manifold = cornersAreManifold(corner_mask) &&
        std::all_of(cs.begin(), cs.end(),
                [](SimplexTree<N>* o){ return o->isManifold(); }) &&
    ...
    leaf->rank = std::accumulate(cs.begin(), cs.end(), (unsigned)0,
            [](unsigned a, SimplexTree<N>* b){ return std::max(a, b->rank());} );
...
template <unsigned N>
double SimplexTree<N>::findVertex(unsigned index)
{
    assert(leaf != nullptr);
...
template <unsigned N>
typename SimplexTree<N>::Vec XTree<N>::vert(unsigned i) const
{
    assert(leaf != nullptr);
...
template <unsigned N>
std::shared_ptr<IntersectionVec<N>> SimplexTree<N>::intersection(
...
template <unsigned N>
Interval::State SimplexTree<N>::cornerState(uint8_t i) const
{
    switch (type)
    {
...
template <unsigned N>
bool SimplexTree<N>::isManifold() const
{
    assert(!isBranch());
    switch (type)
    {
...
template <unsigned N>
uint8_t SimplexTree<N>::cornerMask() const
{
    assert(!isBranch());
    switch (type)
    {
...
template <unsigned N>
unsigned SimplexTree<N>::rank() const
{
    assert(!isBranch());
    switch (type)
    {
...
template <unsigned N>
SimplexTree<N>::Root::Root(XTree<N>* ptr)
    : ptr(ptr)
{
    // Nothing to do here
}
...
template <unsigned N>
SimplexTree<N>::Root::Root(Root&& other)
{
    *this = std::move(other);
}

template <unsigned N>
typename SimplexTree<N>::Root& XTree<N>::Root::operator=(Root&& other)
{
    ptr = other.ptr;
    other.ptr = nullptr;
...
template <unsigned N>
void SimplexTree<N>::Root::reset(ProgressCallback progress_callback)
{
    ptr = nullptr;
...
template <unsigned N>
void SimplexTree<N>::Root::claim(ObjectPool<XTree<N>>& pool)
{
...
template <unsigned N>
void SimplexTree<N>::Root::claim(ObjectPool<XTree<N>::Leaf>& pool)
{
    pool.release(leafs);
}
