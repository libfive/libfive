//  File include/oglplus/enums/single_face.hpp
//
//  Automatically generated file, DO NOT modify manually.
//  Edit the source 'source/enums/oglplus/single_face.txt'
//  or the 'source/enums/make_enum.py' script instead.
//
//  Copyright 2010-2015 Matus Chochlik.
//  Distributed under the Boost Software License, Version 1.0.
//  See accompanying file LICENSE_1_0.txt or copy at
//  http://www.boost.org/LICENSE_1_0.txt
//
#include <oglplus/enumerations.hpp>

namespace oglplus {

/// Enumeration SingleFace
/**
 *  @ingroup oglplus_enumerations
 */
OGLPLUS_ENUM_CLASS_BEGIN(SingleFace, GLenum)
#include <oglplus/enums/single_face.ipp>
OGLPLUS_ENUM_CLASS_END(SingleFace)

#if !OGLPLUS_NO_ENUM_VALUE_NAMES
#include <oglplus/enums/single_face_names.ipp>
#endif

#if !OGLPLUS_NO_ENUM_VALUE_RANGES
#include <oglplus/enums/single_face_range.ipp>
#endif

} // namespace oglplus
